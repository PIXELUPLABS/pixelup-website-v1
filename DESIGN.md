...
| `tracking-clients-heading` | `-0.05em` | Clients-page 40px heading, sourced from the approved Figma layout |
| `tracking-clients-description` | `-0.013623em` | Clients-page 16px description, equivalent to the supplied -0.217969px tracking |
...
- **Sidebar width:** hero/info sidebars use a fixed desktop width with no min/max clamp. The default remains `desk:w-[405px] desk:shrink-0` across `LeftNav`, `BlogSidebar`, `BlogPostSidebar`, and `CaseSidebar`; the approved `/clients` Figma variant is the sole exception at `desk:w-[431px]`. This replaced a viewport-relative formula (`desk:w-[26%] desk:min-w-[340px] desk:max-w-[460px]`, and `desk:w-[calc(26%-10.4px)]` on shell-less pages, where the `-10.4px` compensated for the homepage's missing `p-5`). Keep these widths fixed and don't reintroduce percentages.
